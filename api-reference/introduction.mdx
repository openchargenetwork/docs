...
| `/capabilities` | GET | Authenticated | Partner-specific capabilities and permissions |
...
| `/transfer/webhook` | POST | `transfer.webhook` | Receive signed transfer proofs |
| `/orders/create` | POST | `orders.create` | Merchant Create and sign an order ( Gateways store order and generate txid.) |
| `/orders/{orderId}/status` | GET | `orders.status` | Get order payment status |
| `/verify/{txid}` | GET | `verify.txid` | Verify a transaction effected by /transfer/create |
