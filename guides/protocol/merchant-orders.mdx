...
└──────┬───────┘                      └──────┬───────┘
       ...
       │────────────────────────────────────>│
       │                                     │
       │  2. Merchant signs order            │
       │                                     │
       │  3. Returns signed order + URLs     │
       │<────────────────────────────────────│
       │                                     │
       │  4. Payment app processes payment   │
       │                                     │
```
...
**For Payment Apps:**
- Verify the signature using the merchant's public key from their [metadata](/api-reference/endpoint/metadata)
- Check `expiresAt` before processing
- Use the status URL to confirm payment or listen on [transfer/webhook](/api-reference/endpoint/transfer-webhook)
...
```
┌──────────────┐     ┌──────────────────┐     ┌──────────────┐
│     User     │     │ Payment Gateway  │     │   Merchant   │
│   (Wallet)   │     │   (Processes)    │     │    (POS)     │
└──────┬───────┘     └────────┬─────────┘     └──────┬───────┘
       │                      │                      │
       │  1. Open wallet app  │                      │
       │─────────────────────>│                      │
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────────┐
│     User     │     │   Merchant   │     │ Merchant Gateway │
...
- Implement [transfer/webhook](/api-reference/endpoint/transfer-webhook) for reliable payment confirmation
...
Payment apps and gateways verify signatures using the merchant's public key from their [metadata](/api-reference/endpoint/metadata).
