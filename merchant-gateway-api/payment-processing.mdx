...
|--------|----------|--------|
| Hosted checkout | `/orders/checkout` | redirect to your own payment webapp|
...
The most common flow for e-commerce merchants. You simply redirect the enduser to your hosted payment webapp.

see [Payment Gateway Api](/payment-gateway-api/introduction) for how to implement a Payment Gateway Webapp.

```
Customer → Merchant → Your backend → Customer → Your payment webapp → Your backend → Merchant
```
...
app.post('/orders/checkout', signatureAuth, async (req, res) => {
  ...
  res.json({
    redirect_url: `https://pay.yourgateway.com/checkout/${merchantOrder.id}`
  });
});

// 2. Display payment page in your Payment webapp
app.get('/checkout/:orderId', async (req, res) => {
  const merchantOrder = await db.getOrder(req.params.orderId);

  if (!merchantOrder || merchantOrder.status !== 'pending') {
  ...
  res.render('checkout', {
    merchantOrder,
    merchant: await db.getMerchantByOcid(merchantOrder.order.ocid)
  });
});

// 3. Your payment webapp should now collect the payment and notify the merchant backend via /transfer/create
...
Process transfer requests from your own mobile app or web wallet.  When your webapp or mobile app sends calls this endpoint, ensure the payment is or has been collected, eg by deduction balance, visa etc.
